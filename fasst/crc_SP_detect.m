...
fhc = fqcut(1)/(D.fsample/2); % giopia: fhc = fqcut(1)/(D.fsample/3);
flc = fqcut(2)/(D.fsample/2); % giopia: flc = fqcut(2)/(D.fsample/3);
order=4;
...
if ~crcdef.threshold
    ...
    if handles.analyse==3 || handles.analyse==2
        if isfield(Dor.CRC, 'score') && size(Dor.CRC.score,1)>4
            [Dthresh, TPOIthresh]=crc_extractSW_v2(Dor,handles.analyse,crcdef.stagethresh,scorer,0);
        else
